...
The first DNA sequencing methods were developed in the '70s. 
With the invention of the thermal cycler, the new Sanger's sequencing method, which is referred to as first-generation sequencing (FGS), enables automatization of the sequencing process~\cite{quail2012tale}. 
Next-generation sequencing (NGS) has brought technological advancements have facilitated the production of a huge amount of sequencing data. 
There are two types of sequencing technologies available, which includes sequencing by synthesis (SBS) and sequencing by hybridization and ligation (SBL) approaches.   
